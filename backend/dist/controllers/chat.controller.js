...
export const removeUserFromGroup = async (req, res, next) => {
    try {
        ...
        conversation.participants = conversation.participants.filter(p => p.toString() !== userToRemove);
        ...
        if (conversation.admins?.some(admin => admin.equals(userToRemoveObjectId))) {
            conversation.admins = conversation.admins.filter(a => !a.equals(userToRemoveObjectId));
        }
        await conversation.save();
        // Create system message
